...
import { Text, View, StyleSheet, RefreshControl, Clipboard, ActivityIndicator } from 'react-native';
...
const LayoutInfoWallet = ({
    ...
    const onSelectCoin = () => {
        if (isCoin) {
            ...
            let propsData = getPropData(data, "image", "currency", CurrencyActive, (item) => handleActive(item))
        ...
        if (isCoin && IsActive === "C") {
            ...
            pushSingleScreenApp(componentId, WITHDRAW_COIN_SCREEN, {
                ...
                dataInfo: {
                    amount: {
                        title: "AMOUNT".t(),
                        value: formatTrunc(currencyList, get(item, "amount"), get(item, "currency"))
                    },
                    address: {
                ...
                !isHistoryTransaction ? <TopBarWallet
                    ...
                    renderItem={
                        ...
                        >
                            ...
                            <TextFnx weight={"bold"} space={7} value={`${get(InfoCoin, "currency")} - ${get(InfoCoin, "name")}`} />
                            <TextFnx value={formatCurrency(get(InfoCoin, "available"), get(InfoCoin, "currency"), currencyList)} color={colors.green} weight={"500"} size={20} />
                        </Layout>
                    }
                >
                </TopBarWallet>
