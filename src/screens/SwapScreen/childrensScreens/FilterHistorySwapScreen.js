...
import { fullHeight, fullWidth, listenerEventEmitter, removeEventEmitter, emitEventEmitter, get, isArray, size, createAction, jwtDecode, getCurrentDate, getOneMonthAgoDate, getPropsData } from '../../../configs/utils';
...
const FilterHistorySwapScreen = ({
    ...
    const [ListPay, setListPay] = useState(orderBy(uniqBy(pairs, 'symbol'), ['symbol'], ['asc']));
    const [Listget, setListget] = useState(orderBy(uniqBy(pairs, 'paymentUnit'), ['paymentUnit'], ['asc']));
    ...
    const [WalletTypeActive, setWalletTypeActive] = useState({name:"FIAT".t(),value:"1"});
    const [StatusActive, setStatusActive] = useState({name:"",value:""})
    const [SelectCoinActive, setSelectCoinActive] = useState({symbol:"",name:"",image:""});
    useEffect(() => {
        jwtDecode().then(user => {
            if(get(user,"id")){
    ...
    const onSelectCurrency = () => {
        let data = orderBy([...fiatsWalletType,...coinsWalletType], ['symbol'], ['asc']);
        let propsData = getPropData([{symbol:"ALL".t(),image:"",name:""},...data], "image", "symbol", symbol, (item) => handleActiveCurrency(item))
    ...
    const onWalletType =()=>{
        let walletTypes = [{name:"FIAT".t(),value:"1"},{name:"Coin",value:"2"}]
        let propsData = getPropsData(walletTypes,"", "name", get(WalletTypeActive,"name"), (item) => handleActiveWalletType(item),false)
        showModal(PICKER_SEARCH, propsData)
    }
    ...
    const onStatus =()=>{
        ...
        let propsData = getPropsData(statusList,"", "name", get(StatusActive,"name"), (item) => handleActiveStatus(item),false)
        showModal(PICKER_SEARCH, propsData)
    }
    ...
    const onSelectCoin =()=>{
        if(get(WalletTypeActive,"value") == 1){
            let propsData = getPropData(fiatsWalletType, "image", "symbol", get(SelectCoinActive,"symbol"), (item) => handleActiveSelectCoin(item))
            showModal(PICKER_SEARCH, propsData)
        }else{
            let propsData = getPropData(coinsWalletType, "image", "symbol", get(SelectCoinActive,"symbol"), (item) => handleActiveSelectCoin(item))
            showModal(PICKER_SEARCH, propsData)
        }
    ...
    return (<>
        {HiddenShow && <View style={[stylest.container, {
            ...
            >
                <ItemFilter
                    ...
                    onPressFirst={() => showModal(CALENDAR_SCREEN, {
                        date: 'startDate',
                        currentDate: StartDateData
                    }, true)}
                    onPressSecond={() => showModal(CALENDAR_SCREEN, {
                        date: 'endDate',
                        currentDate: EndDateData
                    }, true)}
                />
                {!isHistoryTransaction &&<ItemFilter
                    valueFirst={symbol}
                    buttonRight
